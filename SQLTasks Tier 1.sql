...
SELECT subquery.facility_name, subquery.revenue
FROM
...
SELECT m.memid, m.surname, m.firstname, (mm.firstname || ' ' || mm.surname) AS recommended_by
FROM Members AS m
...
SELECT b.facid, f.name AS facility_name, COUNT(DISTINCT b.memid) AS member_usage
FROM Bookings AS b
INNER JOIN Facilities AS f 
    ON b.facid = f.facid
WHERE b.memid != 0
...
SELECT 
    ...
    COUNT(DISTINCT b.memid) AS usage_by_month
FROM Bookings AS b
INNER JOIN Facilities AS f 
    ON b.facid = f.facid
WHERE b.memid != 0
